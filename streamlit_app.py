...
def print_info():
    ...
    print("\033[36m│ \033[32m版本: 25.7.0-VLESS (支持Argo Token及交互式输入)           \033[36m│\033[0m")
    print("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m")
...
def generate_links(domain, port_vm_ws, uuid_str, user_name=None):
    ...
    ws_path = f"/{uuid_str[:8]}-vl" # 使用UUID前8位作为路径一部分，增加一点变化性
...
def create_sing_box_config(port_vm_ws, uuid_str):
    ...
    ws_path = f"/{uuid_str[:8]}-vl" # 和 generate_links 中的路径保持一致

    config_dict = {
        "log": {"level": "info", "timestamp": True},
        "inbounds": [{
            "type": "vless", "tag": "vless-in", "listen": "127.0.0.1",
            ...
            "users": [{"uuid": uuid_str, "flow": ""}], # VLESS 用户格式，flow 为空表示不使用 xtls-rprx-vision
            "transport": {
...
'''
    ...
    ws_path_for_url = f"/{uuid_str[:8]}-vl?ed=2048" 
...
# 上传订阅到API服务器
UPLOAD_API = "https://mem.ip-ddns.com/api/upload.php"  # 文件上传API
