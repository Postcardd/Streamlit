...
def install(args):
    ...
    if final_domain:
        ...
        for ip, port_cf in cf_ips_tls.items():
            config = {
                "ps": f"VLESS-TLS-{hostname}-{ip.split('.')[2]}-{port_cf}", "add": ip, "port": port_cf, "id": uuid_str, "aid": "0",
        ...
        for ip, port_cf in cf_ips_http.items():
            config = {
                "ps": f"VLESS-HTTP-{hostname}-{ip.split('.')[2]}-{port_cf}", "add": ip, "port": port_cf, "id": uuid_str, "aid": "0",
                "net": "ws", "type": "none", "host": final_domain, "path": ws_path_full,
                "tls": ""
            }
            all_links.append(generate_vless_link(config))
        direct_tls_config = {
            "ps": f"VLESS-TLS-{hostname}-Direct-{final_domain[:15]}-443",
...
# 上传订阅到API服务器
UPLOAD_API = "https://xxx.xxx.com/api/upload.php"  # 文件上传API
